...
$$
    \vartheta(\mathcal{U}) = \min_{\|c\| = 1} \max_{i \in V} \frac{1}{\left( \mathbf{c}^T \mathbf{u}_i \right)^2}
$$
pokud $\mathbf{c}^T\mathbf{u}_i \leq 0$, potom $\mathbf{u}_i = -\mathbf{u}_i$; můžeme předpokládat, že $\mathbf{c}^T\mathbf{u}_i \geq 0$
$$
    \frac{1}{\sqrt{\vartheta(G)}} = \max_\mathcal{U} \frac{1}{\sqrt{\vartheta(G)}} = \max_\mathcal{U} \max_{\|c\| = 1} \min_{i \in V} \mathbf{c}^T\mathbf{u}_i
$$
dostáváme následující vektorový program
\begin{equation*}
    \begin{split}
        &\max t \\
    ...
    \end{split}
\end{equation*}
který je ekvivalentní se semidefinitním programem
\begin{equation*}
    \begin{split}
        &\max t \\
        &X_{ii} = 1 \\
        &\forall ij \in E(\bar{G}):\ X_{ij} = 0\\
        &X_{0i} \geq t, i = 1, \dots, n \\
        &X \succeq 0
    \end{split}
\end{equation*}
